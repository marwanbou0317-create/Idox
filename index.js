...
async function onMessage(event, api) {
  ...
  if (!body.startsWith(P) && menu.hasSession(event.threadID, event.senderID)) {
    const handled = await menu.handleSession(event, api);
    if (handled) return;
  }
  ...
  // ── القائمة التفاعلية ──────────────────────────────────────────────
  if (cmd === 'قائمة' || cmd === 'menu' || cmd === 'قوائم') {
    if (!admin.isAdmin(senderID)) return adminOnly();
    return menu.show(event, api);
  }
...
async function startBot() {
  ...
  login({
    ...
    // ── تخزين معرّف البوت لنظام القائمة ──────────────────────
    try {
      ...
      log.error('getCurrentUserID: ' + e.message);
    }
    ...
    api.listenMqtt((listenErr, event) => {
      ...
      // ── حماية الكنيات: استعادة عند التغيير ─────────────────
